...
export const GET_DELETE_PARTICPANT_SUCCESS = 'GET_DELETE_PARTICPANT_SUCCESS';
export const GET_DELETE_PARTICPANT_FAILURE = 'GET_DELETE_PARTICPANT_FAILURE';
...
export function fetchDeleteParticipant(uid,token) {
  return async dispatch => {
    try {
      const res = await fetch(`${host}/v2/users/deleteparticipant`, {
        method: 'POST',
        body: JSON.stringify(uid),
        headers: new Headers({ 
          'Content-Type': 'application/json',
      ...
      if (res.status === 200) {
        return dispatch({
          type: GET_DELETE_PARTICPANT_SUCCESS,
          data,
          status: res.status,
        });
      } else if (res.status === 400) {
        return dispatch({
          type: GET_DELETE_PARTICPANT_FAILURE,
          data,
          status: res.status,
        });
      }
